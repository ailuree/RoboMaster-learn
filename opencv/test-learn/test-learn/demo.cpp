...
void demo_imgGet(string str) {
	...
	Mat image(100, 100, CV_8U);//未指定灰度图的像素值时，默认各点为205

	imshow("image0", image);


	Mat image2(100, 100, CV_8UC3, Scalar(0, 0, 255));
	imshow("image2", image2);


	Mat image1 = imread(str);  //读取图像；
	if (image1.empty())
	{
		cout << "读取错误" << endl;
		return;
	...
	cout << "图像0的行数为： " << image.rows << endl;  //获取图像的高度，行数；
	cout << "图像0的列数为： " << image.cols << endl;  //获取图像的宽度，列数；
	cout << "图像0的通道数为： " << image.channels() << endl;  //获取图像的通道数，彩色图=3，灰度图=1；
	cout << "图像0的尺寸为： " << image.size << endl;  //获取图像的尺寸，行*列；
	cout << "图像0的各点像素值为：" << int(image.at<uchar>(0, 0)) << endl;

	cout << "图像1的行数为： " << image1.rows << endl;  //获取图像的高度，行数；
	cout << "图像1的列数为： " << image1.cols << endl;  //获取图像的宽度，列数；
	cout << "图像1的通道数为： " << image1.channels() << endl;  //获取图像的通道数，彩色图=3，灰度图=1；
	cout << "图像1的尺寸为： " << image1.size << endl;  //获取图像的尺寸，行*列；


	cout << "图像2的行数为： " << image2.rows << endl;  //获取图像的高度，行数；
	cout << "图像2的列数为： " << image2.cols << endl;  //获取图像的宽度，列数；
	cout << "图像2的通道数为： " << image2.channels() << endl;  //获取图像的通道数，彩色图=3，灰度图=1；
	cout << "图像2的尺寸为： " << image2.size << endl;  //获取图像的尺寸，行*列；
	cout << "图像2的各点像素通道1为：" << int(image2.at<Vec3b>(0, 0)[0]) << endl;//第一位置的像素点的第一通道
